...
fn test_rg_json_streaming() {
    ...
    // We only need "match" events for sd2 to work? 
    // sd2's stream_rg_json_ndjson uses DeinterleavingSink, which likely expects begin/end or at least match events with paths.
